...
model {
  ...
  for (s in 1:Ns) {
        ...
        // initialise q
        real qv[maxTrials+1,n_thetaz];
          
        // initialise q
        qv = rep_array(.75,maxTrials+1,n_thetaz);
            ...
            for (i in 1:subNtrials[b,s]) {
                ...
                real PthetaCONDx[n_thetaz];
                real Vtheta[n_thetaz];
                real Pstate1CONDx_s;
                real Pstate2CONDx_s;
                real choice;
                real PcorCONDx_s;
                ...
                // calculate posterior over coherence
                for (kk in 1:n_thetaz) {
                    PthetaCONDx[kk]= PMCONDx[n_thetaz+1-kk]+PMCONDx[n_thetaz+kk];
                }
                
                // combine with Q-values
                for (kk in 1:n_thetaz) {
                    Vtheta[kk]= PthetaCONDx[kk]*qv[i,kk];
                }
                
                // generate confidence in other
                PcorCONDx_o = sum(Vtheta);
                ...
                if (self[b,i,s] == 1) {
                  ...
                  u= alpha[s]*d;
                  for (kk in 1:n_thetaz) {
                    qv[i+1,kk]= qv[i,kk]+u*PthetaCONDx[kk];
                  }
                }
