        ...
        k[1] = sigma[s];
            ...
            for (i in 1:subNtrials[b,s]) {
                ...
                for (kk in 1:n_thetaz) {
                    Px_sCONDstate_d1[kk] = exp(normal_lpdf(y[b,i,s]|-1*v_thetaz[kk],sigma[s]));
                    Px_sCONDstate_d2[kk] = exp(normal_lpdf(y[b,i,s]|+1*v_thetaz[kk],sigma[s]));
                }
                ...
                // sigmoid
                if (self[b,i,s] == 1) {
                  pgamble = inv_logit(beta0[s] + beta1_s[s]*(EVr - rs[b,i,s]));
                } else if (self[b,i,s] == 0) {
                  pgamble = inv_logit(beta0[s] + beta1_o[s]*(EVr - rs[b,i,s]));
                }
                ...
                for (kk in 1:n_dthetaz) {
                  Px[kk]= exp(normal_lpdf(y[b,i,s]|v_dthetaz[kk],sigma[s])) * Pk * Pd;
                } 
                term1= Pd/sum(Px);
                ...
                for (kk in 1:n_thetaz) {
                  ...
                  term2= Pk;
                  // term 3
                  term3= exp(normal_lpdf(y[b,i,s]|-v_thetaz[kk],sigma[s]));
                  // term 4
                  // calculate derivative for case where k=r=-1, dP(r=-1|k=-1, theta, x_s) / dsigma
                  nominator= -v_thetaz[kk] * exp( (-(v_thetaz[kk]- (k[i]/sigma[s])* rho[s]* (y[b,i,s]+v_thetaz[kk]) )^2) / (2* (1-rho[s]^2)* k[i]^2));
    ...
    for (b in 1:Nblocks) {
       ...
       for (i in 1:maxTrials) {
          ...
          if (i <= subNtrials[b,s]) {
                ...
                // likelihood of y_s given state
                for (kk in 1:n_thetaz) {
                    Px_sCONDstate_d1[kk] = exp(normal_lpdf(x|-1*v_thetaz[kk],sigma[s]));
                    Px_sCONDstate_d2[kk] = exp(normal_lpdf(x|+1*v_thetaz[kk],sigma[s]));
                }
                ...
                for (kk in 1:n_thetaz) {
                      ...
                      curr_mu_d1_a1 = -1*v_thetaz[kk] + (k[i]/sigma[s]) * rho[s] * (x - -1*v_thetaz[kk]);
                      curr_sigma_d1_a1 = sqrt((1-rho[s]^2) * (k[i]^2));
                      Pa_oCONDstate_x_s_d1_a1[kk] = 0 + 1*normal_cdf(0,curr_mu_d1_a1,curr_sigma_d1_a1);
                      
                      curr_mu_d1_a2 = -1*v_thetaz[kk] + (k[i]/sigma[s]) * rho[s] * (x - -1*v_thetaz[kk]);
                      curr_sigma_d1_a2 = sqrt((1-rho[s]^2) * (k[i]^2));
                      Pa_oCONDstate_x_s_d1_a2[kk] = 1 + -1*normal_cdf(0,curr_mu_d1_a2,curr_sigma_d1_a2);
                      
                      curr_mu_d2_a1 = +1*v_thetaz[kk] + (k[i]/sigma[s]) * rho[s] * (x - +1*v_thetaz[kk]);
                      curr_sigma_d2_a1 = sqrt((1-rho[s]^2) * (k[i]^2));
                      Pa_oCONDstate_x_s_d2_a1[kk] = 0 + 1*normal_cdf(0,curr_mu_d2_a1,curr_sigma_d2_a1);
                      
                      curr_mu_d2_a2 = +1*v_thetaz[kk] + (k[i]/sigma[s]) * rho[s] * (x - +1*v_thetaz[kk]);
                      curr_sigma_d2_a2 = sqrt((1-rho[s]^2) * (k[i]^2));
                      Pa_oCONDstate_x_s_d2_a2[kk] = 1 + -1*normal_cdf(0,curr_mu_d2_a2,curr_sigma_d2_a2);
                ...
                if (self[b,i,s] == 1) {
                  ...
                  // term 1
                  for (kk in 1:n_dthetaz) {
                    Px[kk]= exp(normal_lpdf(x|v_dthetaz[kk],sigma[s])) * Pk * Pd; 
                  } 
                  term1= Pd/sum(Px);
                  // terms 2-4 are found by summing over theta 
                  for (kk in 1:n_thetaz) {
                    ...
                    term2= Pk;
                    // term 3
                    term3= exp(normal_lpdf(x|-v_thetaz[kk],sigma[s]));
                    // term 4
                    // calculate derivative for case where k=r=-1, dP(r=-1|k=-1, theta, x_s) / dsigma
                    nominator= -v_thetaz[kk] * exp( (-(v_thetaz[kk]- (k[i]/sigma[s])* rho[s]* (x+v_thetaz[kk]) )^2) / (2* (1-rho[s]^2)* k[i]^2));
                    denominator= sqrt(2* pi()* (1-rho[s]^2))* k[i]^2;
                    term4= nominator/denominator;
                    combine[kk]= term2* term3* term4;
                  }
